        ...
        <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border z-20 p-3">
          ...
          <div className="flex gap-2 mb-2 text-xs">
            <button
              ...
              className={`px-2 py-1 rounded ${type === "users" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600"}`}
            >
              Пользователи
            </button>
            <button
              onClick={() => setType("posts")}
              className={`px-2 py-1 rounded ${type === "posts" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600"}`}
            >
              Посты
            </button>
          </div>
          ...
          <div className="max-h-72 overflow-y-auto space-y-1">
            {loading && <p className="text-xs text-gray-400 py-2 text-center">Поиск...</p>}
            ...
            {!loading && type === "users" &&
              userResults.map((u) => (
                <Link
                  ...
                  className="block px-2 py-1.5 rounded hover:bg-gray-50 text-sm"
                >
            ...
            {!loading && type === "posts" &&
              postResults.map((p) => (
                <Link
                  ...
                  className="block px-2 py-1.5 rounded hover:bg-gray-50 text-sm"
                >
                  <div className="font-medium truncate">{p.title || "Без названия"}</div>
                  <div className="text-gray-400 text-xs truncate">{p.content}</div>
                </Link>
              ))}
            ...
            {!loading &&
              ...
              ((type === "users" && userResults.length === 0) ||
                ...
                <p className="text-xs text-gray-400 py-2 text-center">Ничего не найдено</p>
              )}
          </div>
        </div>
      )}
